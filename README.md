...
| 우선순위 | 클립 | 언제 나오나 |
|---|---|---|
| 1 (가장 우선) | `walkRight` / `walkLeft` | 말풍선 아래 점을 눌러 다른 세션으로 전환한 직후 1.1초간 |
| 2 | `jump` | 답변이 막 도착한 직후 3.5초간 |
| 3 | `lookRight` / `lookLeft` | 마우스가 펫 위에 있는 동안 — 커서가 펫 중심보다 오른쪽/왼쪽 어디 있는지에 따라 |
| 4 (평소 상태) | `wave` | Claude Code가 `Notification`(확인 필요) 상태일 때 |
| 4 | `ponder` | `UserPromptSubmit`(생각 중) 상태일 때 |
| 4 | `working` → `poke` | `PreToolUse`/`PostToolUse`(작업 중) 상태일 때. 15초 넘게 이어지면 `poke`(더 적극적으로 건드림)로 전환 |
| 4 | `idle` → `sulky` | `Stop`/`SessionStart`(대기)거나 돌아가는 세션이 아예 없을 때. 60초 넘게 이어지면 `sulky`(시무룩)로 전환 |

즉 "점 클릭"과 "답변 도착"은 잠깐 끼어드는 연출이라 다른 무엇보다 우선하고,
그다음은 "마우스가 올라와 있는지", 그것도 아니면 지금 Claude Code 상태
그대로를 보여줍니다. 1~3순위가 끝나면 4순위(원래 상태)로 자연히 돌아옵니다.
